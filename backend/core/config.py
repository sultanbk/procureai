...
LLM_CALL_TIMEOUT_SECONDS = get_int("LLM_CALL_TIMEOUT_SECONDS", 240)
PIPELINE_MAX_LLM_CALLS = get_int("PIPELINE_MAX_LLM_CALLS", 100)
PIPELINE_TIMEOUT_SECONDS = get_int("PIPELINE_TIMEOUT_SECONDS", 600)
MAX_TOKENS_PER_AUDIT = get_int("MAX_TOKENS_PER_AUDIT", 500000)
LLM_RPM_LIMIT = get_int("LLM_RPM_LIMIT", 60)
LLM_TPM_LIMIT = get_int("LLM_TPM_LIMIT", 100000)
...
# SynaptAI Context Substrate Configuration
CONTEXT_SUBSTRATE_ENABLED = get_bool("CONTEXT_SUBSTRATE_ENABLED", True)
CONTEXT_SUBSTRATE_MODE = os.getenv("CONTEXT_SUBSTRATE_MODE", "auto").lower()  # auto | live | mock
CONTEXT_SUBSTRATE_URL = os.getenv("CONTEXT_SUBSTRATE_URL", "http://localhost:8004")
CONTEXT_SUBSTRATE_INGEST_URL = os.getenv("CONTEXT_SUBSTRATE_INGEST_URL", "http://localhost:8001")
CONTEXT_SUBSTRATE_API_KEY = os.getenv("CONTEXT_SUBSTRATE_API_KEY", "")
CONTEXT_SUBSTRATE_PROVIDER_ID = os.getenv("CONTEXT_SUBSTRATE_PROVIDER_ID", "procureai")
CONTEXT_SUBSTRATE_EXTRACTION_DENSITY = os.getenv("CONTEXT_SUBSTRATE_EXTRACTION_DENSITY", "medium")  # low | medium | high
CONTEXT_SUBSTRATE_EXTRACTION_MODE = os.getenv("CONTEXT_SUBSTRATE_EXTRACTION_MODE", "balanced")  # fast | balanced | max_extraction | deep | fact_dense

# Trident MCP Registered Agent & Substrate Settings (Prodapt IPL SDK)
SYNAPT_MCP_URL = os.getenv("SYNAPT_MCP_URL", "https://beta.synapt.ai/api/mcp").rstrip("/")
SYNAPT_PROVIDER_ID = os.getenv("SYNAPT_PROVIDER_ID", CONTEXT_SUBSTRATE_PROVIDER_ID)
SYNAPT_AGENT_CLIENT_ID = os.getenv("SYNAPT_AGENT_CLIENT_ID", "client_procureai_auditor_s2s")
SYNAPT_AGENT_CLIENT_SECRET = os.getenv("SYNAPT_AGENT_CLIENT_SECRET", "")
SYNAPT_AGENT_TOKEN = os.getenv("SYNAPT_AGENT_TOKEN", CONTEXT_SUBSTRATE_API_KEY)
SYNAPT_VERIFY_SSL = get_bool("SYNAPT_VERIFY_SSL", False)
